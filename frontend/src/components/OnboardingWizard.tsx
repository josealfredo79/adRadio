      ...
      <div className="w-full max-w-lg rounded-2xl bg-white shadow-2xl overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-brand-600 to-brand-500 px-6 py-5 text-white">
          <div className="flex items-center justify-between">
            ...
            <button onClick={onClose} className="text-brand-200 hover:text-white"><X className="h-5 w-5" /></button>
          </div>
          {/* Progress */}
            ...
            {STEPS.map((s, i) => (
              <div key={s.id} className="flex items-center gap-2">
                <div className={`flex h-7 w-7 items-center justify-center rounded-full text-xs font-bold transition-all ${
                  completed.includes(s.id) ? 'bg-white text-brand-600' :
                  step === s.id ? 'bg-brand-400 text-white ring-2 ring-white' :
                  'bg-brand-700/50 text-brand-200'
                }`}>
                  {completed.includes(s.id) ? <CheckCircle2 className="h-4 w-4" /> : s.id}
                </div>
                {i < STEPS.length - 1 && (
                  <div className={`h-0.5 w-10 transition-all ${completed.includes(s.id) ? 'bg-white' : 'bg-brand-700/50'}`} />
                )}
              </div>
            ))}
          </div>
        ...
        <div className="p-6">
          {stepError && <p className="mb-4 rounded-lg bg-red-50 px-3 py-2 text-sm text-red-600">{stepError}</p>}

          {/* Step 1: Knowledge Base */}
          {step === 1 && (
            <div className="space-y-4">
              <div>
                <h3 className="font-semibold text-gray-900">Sube tu base de conocimiento</h3>
                <p className="mt-1 text-sm text-gray-500">Un PDF, TXT o DOCX con info de tu negocio. El bot la usará para responder clientes.</p>
              </div>
              <div
                className="flex cursor-pointer flex-col items-center justify-center rounded-xl border-2 border-dashed border-gray-300 px-6 py-8 hover:border-brand-400 hover:bg-brand-50 transition-colors"
                onClick={() => fileRef.current?.click()}
              >
                <Upload className="h-8 w-8 text-gray-400" />
                <p className="mt-2 text-sm font-medium text-gray-600">{kbFile ? kbFile.name : 'Haz clic para seleccionar archivo'}</p>
                <p className="text-xs text-gray-400">PDF, TXT, DOCX — máx. 10MB</p>
              ...
              <div className="flex items-center justify-between pt-1">
                <button onClick={onClose} className="text-sm text-gray-400 hover:text-gray-600">Omitir por ahora</button>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <div>
                <h3 className="font-semibold text-gray-900">Agrega tu primer contacto</h3>
                <p className="mt-1 text-sm text-gray-500">Ingresa un número de WhatsApp para comenzar a enviar mensajes.</p>
              </div>
              <div>
          ...
          {step === 3 && (
            <div className="space-y-4">
              <div className="text-center py-4">
                ...
                <h3 className="mt-3 font-semibold text-gray-900">¡Todo listo para tu primera campaña!</h3>
                <p className="mt-1 text-sm text-gray-500">Ahora puedes crear y enviar campañas de WhatsApp a tus contactos.</p>
              </div>
              <div className="flex flex-col gap-3">
                <button onClick={finish}
                  className="flex items-center justify-center gap-2 rounded-xl bg-brand-600 px-6 py-3 font-medium text-white hover:bg-brand-700">
