  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Preview */}
        <div className="bg-gray-50 border border-gray-200 rounded-xl p-6 relative overflow-hidden order-2 lg:order-1">
          <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
            <Smartphone size={16} />
            Vista previa en vivo
          </div>
          <div className="bg-white border border-gray-200 rounded-lg h-64 flex items-end justify-end p-4 relative">
            <span className="text-gray-300 text-sm absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
              Tu sitio web
            </span>
            {/* Simulated popup */}
            <div
              className={`absolute bottom-20 ${configData.position === 'left' ? 'left-6' : 'right-6'} w-64 rounded-2xl bg-white shadow-xl border border-gray-100 overflow-hidden`}
            >
              ...
              <div className="px-4 py-3 bg-gray-50">
                <div
                  className="bg-white rounded-r-xl rounded-bl-xl px-3 py-2 text-sm text-gray-700 shadow-sm max-w-[85%]"
                >
                  {configData.greeting}
                </div>
              </div>
        ...
        <div className="space-y-5 order-1 lg:order-2">
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-3">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400">o hex:</span>
              <input
                type="text"
                value={configData.color}
                onChange={e => setForm(prev => ({ ...(prev ?? configData), color: e.target.value }))}
                className="w-24 rounded-lg border border-gray-300 px-2 py-1.5 text-xs font-mono focus:border-brand-500 focus:outline-none"
                maxLength={7}
              />
            </div>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-5 space-y-2">
            ...
            />
            <p className="text-xs text-gray-400 text-right">{configData.greeting.length}/200</p>
          </div>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 rounded-xl bg-brand-500 px-5 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50 transition-colors"
          >
            <Save size={16} />
            {saveMutation.isPending ? 'Guardando...' : 'Guardar cambios'}
          </button>
          {saveMutation.isSuccess && <p className="text-sm text-green-600 font-medium text-center">✓ Widget actualizado</p>}
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="flex items-center gap-2 text-sm px-4 py-2 bg-brand-500 text-white rounded-lg hover:bg-brand-600 disabled:opacity-50 transition"
          >
